	...
	while (*format && n < size - 1)
	{
		...
		else if (*format == 'u')
		{
			if (is_long)  /* 假设 is_long 表示是否需要 long 类型 */
				ft_utoa_printf(buf, &n, size, va_arg(*ap, unsigned long));
			else
				ft_utoa_printf(buf, &n, size, va_arg(*ap, unsigned int));
		}
		else if (*format == 'x' || *format == 'X')
		{
			...
			if (prefix)
				...
				ft_htoa_printf(buf, &n, size, nb, uppercase);
		}
		else if (*format == 'p')
		{
			ptr = va_arg(*ap, void*);
			...
			ft_htoa_printf(buf, &n, size, addr, 0);  /* 强制小写十六进制 */
		}
		else if (*format == 's')
		{
			str = va_arg(*ap, char*);
